...
def test_mongo_retrieval_failure(mongo_test_storage, monkeypatch, caplog):
    ...
    assert (
        f"Error during attempted retrieval of key {key} (namespace = '{mongo_test_storage.namespace}'): {e}"
        in caplog.text
    )
...
def test_mongo_retrieve_keys_error(mongo_test_storage, monkeypatch, caplog):
    ...
    assert keys == []
    assert f"Error during attempted retrieval of all keys from namespace '{mongo_test_storage.namespace}" in caplog.text
...
def test_mongo_update_error(mongo_test_storage, caplog):
    ...
    with patch.object(mongo_test_storage, "collection") as mock_collection:
        ...
        with patch.object(mongo_test_storage, "verify_cache", return_value=False):
            mongo_test_storage.update(key, {"data": 1})
        assert (
            f"Error during attempted update of key {key} (namespace = '{mongo_test_storage.namespace}': {e}"
            in caplog.text
        )
...
def test_mongo_delete_error(mongo_test_storage, caplog):
    ...
    with patch.object(mongo_test_storage, "collection") as mock_collection:
        ...
        mongo_test_storage.delete(key)
        assert (
            f"Error during attempted deletion of key {key} (namespace = '{mongo_test_storage.namespace}'): {e}"
            in caplog.text
        )
...
def test_mongo_delete_all_error(mongo_test_storage, caplog):
    ...
    with patch.object(mongo_test_storage, "collection") as mock_collection:
        ...
        assert (
            f"Error during attempted deletion of all records from namespace '{mongo_test_storage.namespace}': {e}"
            in caplog.text
        )
...
def test_mongo_verify_cache_error(mongo_test_storage, monkeypatch, caplog):
    ...
    e = "DB error"
    key = "some_key"
    monkeypatch.setattr(mongo_test_storage.client.collection, "distinct", raise_error(PyMongoError, e))
