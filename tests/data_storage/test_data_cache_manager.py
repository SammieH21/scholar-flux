...
def test_basic_cache_operations(request, storage, mock_response, mock_cache_storage_data):
    ...
    # Test verify cache
    assert cache_manager.verify_cache(cache_key) is True
...
def test_cache_with_different_response_hashes(mock_response):
    ...
    cache_manager.update_cache(
        cache_key=cache_key,
        response=mock_response,
        processed_response={'original': True}
    )
