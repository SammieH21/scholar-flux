...
def test_basic_key_pattern():
    ...
    string_patterns = {KeyMaskingPattern(name = 'birthdate',
                                         field = field,
                                         pattern = birthdate,
                                         replacement = masking_text)
                       for field in fields}
...
def test_secret_masking():
    masker = SensitiveDataMasker(register_defaults = False)
    assert masker.mask_secret(None) == None
    ...
    assert new_secret and isinstance(new_secret[0].pattern, SecretStr) and new_secret[0].pattern == a_secret # type: ignore

    masker.add_sensitive_string_patterns(name='new_secret', patterns = [a_secret.get_secret_value()], use_regex=False)

    assert masker.mask_text(None) is None and masker.mask_text(1) is 1 #type:ignore

def test_repr():
    masker = SensitiveDataMasker(register_defaults = True)
    assert 'SensitiveDataMasker(patterns=MaskingPatternSet(...))' == repr(masker)
...
def test_pattern_set():
    ...
    item = (2, )
    with pytest.raises(TypeError) as excinfo:
        pattern_set.update((item, )) # type: ignore
    assert f"Expected a masking pattern, received type {type(item)}" in str(excinfo.value)
