...
def test_request_failed_exception(monkeypatch, caplog):
        ...
        monkeypatch.setattr(coordinator,
                                 'robust_request',
                                 lambda *a, **kw: (_ for _ in ()).throw(RequestFailedException("fail")))
...
#       assert result.data and len(result.data) == 3
    ...
    with requests_mock.Mocker() as m:
        m.get(prepared_request.url, status_code = 200, content = academic_json_response.content, headers={"Content-Type": "application/json"})
        result = coordinator.search(page = 1, from_request_cache= False, from_process_cache=False)
        assert isinstance(result, ProcessedResponse)
        assert result.data and len(result.data) == 3
