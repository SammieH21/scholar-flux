...
def set_default_rate_limiter_interval():
    ...
    The default value is reset after the the current test that that uses this dependency concludes.
    """
...
def test_wait_sleeps_when_needed_real_time(Limiter):
    ...
    specified interval. This function patches time.sleep to verify that the sleep argument is called with
    the intended arguments while recording the time when sleep was last performed.
    """
...
def test_wait_no_sleep_if_enough_time_real_time(Limiter):
    """
    Verifies that the sleep function will not wait if enough time has passed between the last
    call of the `wait()` method.
    """
...
def test_context_manager_calls_wait(mock_sleep):
    """
    Tests the total number of times that the `_wait` helper method is called when
    te rate limiter's min_interval parameter is set to a value of 1.
...
def test_rate_context_manager_temporary_interval(mock_time):
    """
    Tests that the temporary modification of the min_interval attribute with a context manager will
