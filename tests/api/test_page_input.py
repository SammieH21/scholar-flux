...
def test_singular_large_page():
    """
    Test a large page number: This method should work, whether for a small number, or an abnormally large page number
    """
    pages = 50000000
    page_list_input = PageListInput(pages)  # type:ignore
    assert list(page_list_input.page_numbers) == [pages]
...
def test_zero_indexed_page_list():
    ...
    A zero indexed page should be allowed and handled only at the parameter building steps which will determine
    whether page=0 should be allowed given the parameter configuration for the current API Provider
    """
    pages = [0, 1, 2]
    page_list_input = PageListInput(pages)
    assert list(page_list_input.page_numbers) == pages
...
def test_invalid_inputs(caplog):
    """
    Tests whether inputs that include invalid page numbers will raise an error as intended. Floats are flagged
    rather than coerced, and non-numeric strings should also raise errors.
    """
