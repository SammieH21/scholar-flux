...
@pytest.mark.parametrize(('provider', 'expected_type'), [('pubmed', ProviderConfig),
...
def test_match(provider, expected_type):
    ...
    if provider_config:
        ...
        assert provider_from_url.provider_name == provider_config.provider_name

    if provider is None:
        assert provider not in provider_registry
        assert provider_registry.get(provider) is None
