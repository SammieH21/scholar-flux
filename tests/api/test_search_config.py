...
@pytest.mark.parametrize(['provider', 'basename'],
                         [('plos', 'plos'), ('pubmed_efetch', 'nih'),
                          ('pubmed', 'nih'), ('springernature', 'springernature',),
                          ('crossref','crossref'), ('core','core')])
...
def test_api_key_modification():
    ...
    with patch.dict(scholar_flux.api.models.search.config, {pubmed_provider_info.api_key_env_var: another_api_key}):
        ...
        assert pubmed_config.provider_name == 'pubmed' and pubmed_config.api_key == api_key and pubmed_config.api_key != another_api_key
...
def test_search_api_config_dynamic_provider_override(caplog):
    ...
    with patch.dict(scholar_flux.api.models.search.config, {provider_info.api_key_env_var: api_key}):
        ...
        assert pubmed_config.base_url == provider_info.base_url and pubmed_config.provider_name == provider_info.provider_name
    ...
    with caplog.at_level(logging.WARNING):
        ...
        warning_msg = ("The provided base URL resolves to a provider while the provider name, "
