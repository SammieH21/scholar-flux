...
def springer_nature_sample_record():
    """Sample SpringerNature API record for integration testing."""
    return {
        "doi": "10.1234/example",
...
def springer_nature_minimal_record():
    """Minimal SpringerNature record with required fields only."""
    return {
...
def springer_nature_complete_record():
    """Complete SpringerNature record with all fields."""
    return {
...
def test_extract_year_from_publication_date(springer_nature_sample_record):
    """Test year extraction from publicationDate field."""
    year = extract_year(springer_nature_sample_record["publicationDate"])
    assert year == 2026
...
def test_extract_year_iso_datetime():
    """Test year extraction from ISO datetime format."""
    assert extract_year("2024-06-15T10:30:00Z") == 2024


# ==================== Tests for Open Access Validation ====================


def test_validate_open_access_true(springer_nature_sample_record):
    """Test open access boolean conversion for 'true' string."""
    open_access = validate_bool_str(springer_nature_sample_record["openaccess"])
    assert open_access is True


def test_validate_open_access_false():
    """Test open access boolean conversion for 'false' string."""
    assert validate_bool_str("false") is False


def test_validate_open_access_none():
    """Test open access boolean conversion for None."""
    assert validate_bool_str(None) is None
...
def test_extract_nested_springer_nature_primary_url(springer_nature_sample_record):
    """Verifies that `extract_url` correctly traverses Springer Nature lists and dicts to get the first valid URL."""
    url = SpringerNatureFieldMap.extract_url(springer_nature_sample_record, ["url", 0, "value"], ["url", 0], ["url"])
    assert url == "http://example.com/article"


def test_extract_valid_first_valid_from_multi_url_string():
    """Verifies that `extract_url` correctly retrieves the first valid URL from a delimited string of URLs."""
    record = {"url": "https://example1.com/;https://example2.com/;https://example3.com/"}
...
def test_extract_url_fallback_paths():
    """Ensure fallback paths work in order."""
    record = {
...
def test_extract_url_invalid_urls_skipped():
    """Ensure invalid URLs in list are skipped."""
...
def test_springer_nature_transformations(springer_nature_sample_record):
    """Integration test for SpringerNature field transformations."""
    record = springer_nature_sample_record

    year = extract_year(record["publicationDate"])
    assert year == 2026

    open_access = validate_bool_str(record["openaccess"])
    assert open_access is True
...
def test_springer_nature_null_open_access():
    """Test normalization handles null open access field."""
...
def test_springer_nature_empty_url_list():
    """Test normalization handles empty URL list."""
...
def test_springer_nature_single_author():
    """Test normalization handles single author (not in list)."""
