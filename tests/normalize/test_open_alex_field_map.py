...
def openalex_sample_open_access_record():
    """Fixture of a raw OpenAlex mock API response for open access article."""
    return {
...
def openalex_sample_closed_access_record():
    """Raw OpenAlex mock API response for closed access article."""
    return {
...
def openalex_sample_retracted_record():
    """Raw OpenAlex mock API response for retracted article."""
    return {
...
def test_reconstruct_abstract_filters_invalid_positions():
    """Abstract reconstruction skips non-integer positions."""
    record = {
        "abstract_inverted_index": {
            "valid": [0, 2],
            "word": [1],
...
def test_reconstruct_abstract_single_edge_case():
    """Abstract reconstruction skips non-integer positions."""
...
def test_reconstruct_abstract_invalid_type():
    """Abstract reconstruction returns None for non-dict input."""
...
def test_normalize_doi_non_string():
    """DOI normalization returns None for non-string input."""
    record = {"doi": 12345}
...
def test_extract_pmid_non_string():
    """PMID extraction returns None for non-string input."""
    record = {"pmid": 12345}
...
def test_post_process_open_access_record():
    """Full post-processing of open access record with abstract."""
    record = {
...
def test_post_process_closed_access_record():
    """Full post-processing of closed access record without abstract."""
    record = {
        "provider_name": "openalex",
        "doi": "https://doi.org/10.1016/j.example.2024.01.001",
...
def test_post_process_none_values():
    """Post-processing handles None values gracefully."""
    record = {
        "provider_name": "openalex",
        "doi": None,
        "authors": None,
...
def test_post_process_preserves_api_fields():
    """API-specific fields pass through post-processing unchanged."""
    record = {
        "provider_name": "openalex",
...
def test_post_process_retracted_article():
    """Post-processing preserves retraction status."""
    record = {
        "provider_name": "openalex",
...
def test_normalize_record_closed_access(openalex_sample_closed_access_record):
    """Full normalization of closed access record from raw API response."""
...
def test_normalize_record_retracted(openalex_sample_retracted_record):
    """Full normalization of retracted record from raw API response."""
...
def test_normalize_record_minimal(openalex_sample_record_minimal):
    """Full normalization of minimal record with only required fields."""
