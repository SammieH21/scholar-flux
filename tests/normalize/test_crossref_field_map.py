...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tdm/userlicense", False),
        ("/tdm", False),
        ("creativecommons.org/licenses/by-nc/", None),
        ("creativecommons.org/licenses/by-nd/", None),
    ],
)
def test_license_patterns_structure(pattern, expected):
    """Verify LICENSE_PATTERNS has correct structure for key patterns."""
    assert LICENSE_PATTERNS[pattern] is expected
...
def test_extract_journal(journal_input, expected):
    """Journal extraction handles strings, lists, and invalid types."""
...
def crossref_raw_record_elsevier():
    """Raw Crossref record from Elsevier with TDM license (before normalization)."""
    return {
...
def crossref_raw_record_cc_by():
    """Raw Crossref record with CC-BY open access license (before normalization)."""
    return {
...
def crossref_raw_record_minimal():
    """Minimal raw Crossref record fixture with only required fields (before normalization)."""
    return {
        "DOI": "10.1234/test",
    }
...
def test_normalize_record_elsevier(crossref_raw_record_elsevier):
    """Full normalization of Elsevier TDM record."""
...
def test_normalize_record_cc_by(crossref_raw_record_cc_by):
    """Full normalization of CC-BY open access record."""
...
def test_normalize_record_minimal(crossref_raw_record_minimal):
    """Full normalization of minimal Crossref record."""
...
def test_normalize_record_batch(
    ...
    """Batch normalization of multiple Crossref records."""
    records = [
        crossref_raw_record_elsevier,
        crossref_raw_record_cc_by,
        crossref_raw_record_minimal,
