...
def test_formatting(all_path_nodes):
    """
    Tests whether the helper function accommodates a the expected range of inputs including nodes and/or
    a sequence/mapping of nodes as intended.
    """
...
def test_record_map_creation():
    ...
    Each method should result in the same record map structure and not vary.
    We can consider a sequence of nodes to internally map a path to a node value.
    """
    a1 = PathNode.to_path_node("1.a", value=1)
    a2 = PathNode.to_path_node("1.b", value=2)
    a3 = PathNode.to_path_node("1.c", value=3)
...
def test_chain_input_preparation():
    """
    Test robustness and equality of methods used to prepare node inputs for use in practice
    by testing `_prepare_inputs` with a range of both valid and invalid inputs.
    """
...
def test_contains(default_mapping):
    """
    Verifies that key membership is operating as intended. Nodes, paths, and strings should first be coerced
    into paths and subsequently record_indexes to verify whether, first, the record_index exists associated
    with the path.

    If an actual path is specified instead of a record index, then membership testing will existing to the path
    and verify whether the associated path node exists.
    """
