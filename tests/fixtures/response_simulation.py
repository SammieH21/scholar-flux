...
def mock_pubmed_search_data(mock_pubmed_search_json_path) -> dict[str, Any]:
    mock_pubmed_search_data = json.loads(
        mock_pubmed_search_json_path.read_text(encoding='utf-8')
    )
    return mock_pubmed_search_data

@pytest.fixture
def mock_pubmed_fetch_data(mock_pubmed_fetch_json_path) -> dict[str, Any]:
    mock_pubmed_fetch_json_data = json.loads(
        mock_pubmed_fetch_json_path.read_text(encoding='utf-8')
    )
