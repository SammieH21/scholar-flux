...
def mock_pubmed_search_endpoint() -> re.Pattern:
    ...
    mock_pubmed_search_endpoint = re.compile(
        re.escape("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?") + "[a-z0-9&_=%]+"
    )
...
def mock_pubmed_fetch_endpoint() -> re.Pattern:
    ...
    mock_pubmed_fetch_endpoint = re.compile(
        re.escape("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?") + "[a-z0-9&_%]+="
    )
...
def mock_pubmed_search_json_path() -> Path:
    """Defines the path where JSON data is stored too simulate the retrieval of page 3 eSearch data from the PubMed
    API."""
...
def mock_pubmed_fetch_json_path() -> Path:
    """Defines the path where JSON data is stored too simulate the retrieval of page 3 eFetch data from the PubMed
    API."""
...
def mock_pubmed_fetch_data(mock_pubmed_fetch_json_path) -> dict[str, Any]:
    """The data to return when simulating the retrieval of page 3 eFetch data from the PubMed API with the current query
    and requests_mock."""
...
def mock_pubmed_search_response(mock_pubmed_search_data) -> Response:
    ...
    3 of the PubMed eSearch API with the current query."""
...
def mock_pubmed_fetch_response(mock_pubmed_fetch_data) -> Response:
    """A fixture that uses the requests_mock package to simulate the retrieval of a requests.Response instance for page
    3 of the PubMed eFetch API with the current query."""
    # Create a mock object that behaves like a Response instance
    mock_response = Response()
