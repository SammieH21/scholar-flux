...
"""
The scholar_flux.exceptions.import_exceptions module implements exceptions for handling missing dependencies
when using the scholar_flux package. Although several packages are optional, attempting to use functionality
that directly depends on an uninstalled dependency should raise an error.
...
class OptionalDependencyImportError(Exception):
    ...
    def __init__(self, message="Optional Dependency not found"):
        """
        Initializes the foundational OptionalDependencyImportError that forms the basis of more specific error handling
