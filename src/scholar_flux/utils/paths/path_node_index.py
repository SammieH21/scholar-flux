...
The PathNodeIndex is responsible for orchestrating JSON data discovery,
processing, and flattening to abstract JSON data into path-node pairs
indicate the location of terminal values and the path location of the
terminal-values within within a nested JSON data structure.
"""
...
class PathNodeIndex:
    ...
    def __post_init__(self):
        ...
        The index represents the preprocessed json data that has been
        transformed into a dictionary of path-node mappings whereas the
        validated simplifier is then used to flatten the the index into
