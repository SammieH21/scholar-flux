...
This module implements the RecordPathNodeMap and RecordPathChainMap,
respectively to process batches of nodes at a time that all apply to a
single record while allowing speedups to cache when retaining only
terminal nodes via set/dictionary operations.
"""
...
class RecordPathNodeMap(PathNodeMap):
    ...
    This implementation inherits from the PathNodeMap class and
    constrains the allowed nodes to those that begin with a numeric
    record index. Where each index indicates a record and nodes
    ...
    def __init__(
        ...
        **path_nodes: Mapping[str | ProcessingPath, PathNode],
        ...
        This implementation constraints the inputted nodes to a singular
        numeric key index that all nodes must begin with. If nodes are
        provided without the key, then the `record_index` is inferred
        for the inputs.
        """
