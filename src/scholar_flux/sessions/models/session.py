...
class CachedSessionConfig(BaseModel):
    ...
    def validate_cache_directory(cls, v) -> Optional[Path]:
        ...
        raise ValueError(f"The cache_directory parameter expected a path, received a value of a different type ({type(v)}).")
    ...
    def validate_expire_after(cls, v):
        ...
        if isinstance(v, int) and v < 0 and not v == -1:
            raise ValueError(f"The provided integer for the expire_after parameter ({v}) must be greater "
    ...
    def validate_backend_dependency(cls, v):
        ...
        if deps is None:
            ...
            raise ValueError(f"Requests-Cache does not support a backend by the name of {backend}.\n"
    ...
    def validate_backend_filepath(cls, values):
        ...
        if backend in ('filesystem', 'sqlite') and cache_directory is None:
            raise ValueError(f"A filepath must be specified when using the {backend} backend. "
        ...
        if backend not in ('filesystem', 'sqlite') and cache_directory is not None:
            ...
            logger.debug(f"When initialized, the Cached Session Configuration will use the {backend} "
                         f"backend and the path: {cache_path}.")
            if not cache_path.parent.exists():
                logger.warning(f"Warning: The parent directory, {cache_path.parent}, does not exist "
