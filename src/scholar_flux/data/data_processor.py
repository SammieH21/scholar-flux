...
from scholar_flux.utils import (get_nested_data, as_list_1d, nested_key_exists,
                                get_nested_data)

from scholar_flux.data import BaseDataProcessor
import re
...
class DataProcessor(BaseDataProcessor):
    ...
    - None
    """
    def __init__(self, record_keys: Optional[Dict[Any,Any]]=None,
                 header_path: Optional[List]=None,
                 body_path: Optional[List] = None,
                 ignore_records_with: Optional[List] = None,
    ...
    def define_record_keys(self, keys=None)-> Dict[str,str]:
        default_keys={
    ...
    def ignore_record_keys(self,keys=None)->List:
        default_keys = keys or ['.*facets.*','.*facet-value.*']
        return default_keys


    def define_record_path(self, header: Optional[list]=None, body: Optional[list]=None)->Tuple[List,List]:
        """
    ...
    def process_key(self,record: Dict | list | str | int | None, key: str)->Optional[str]:
        """
        Processes a specific key from record by joining non-None values into a string.
    ...
    def process_text(self, record_dict: Dict):
        """
        Processes a record dictionary to extract record and article content, creating a processed record dictionary with an abstract field.
    ...
    def process_record(self, record_dict: Dict):
        """
        ...
        if not record_data:
            logger.debug("Could not retrieve header paths from the record dict")
            # Simplified record data processing using dictionary comprehension
        processed_record_dict = {
            col: self.process_key(record_data,
                                  key) for col, key in self.record_keys.items()
...
if __name__ == '__main__':
    ...
    if not isinstance(sample_json,str):
        ...
        processed = processor.process_page(sample_json) if isinstance(sample_json, list) else None
