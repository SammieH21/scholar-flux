...
class BaseStepContext(BaseModel):
    """
    Base class for step contexts. Passed between workflow steps to communicate the context and history of the
    current workflow before and after the execution of each step.
    """


class BaseWorkflowStep(BaseModel):
    """Base class for workflow steps. Used to define the behavior and actions of each step in a workflow"""
    ...
    def pre_transform(self, ctx: Any, *args, **kwargs) -> Self:
        """
        Defines the optional transformation to the BaseWorkflowStep that can occur before executing
        the workflow step to generate and modify its behavior.
    ...
    def post_transform(self, ctx: Any, *args, **kwargs) -> Any:
        """
        Defines the optional transformation to the results that are retrieved after executing
        the workflow step to modify its output.
    ...
    def _verify_context(self, ctx: Any) -> None:
        """
        Helper method for verifying the context received to ensure that the correct inputs are received before
        step execution.
...
class BaseWorkflow(BaseModel, ABC):
    ...
    def __call__(self, *args, **kwargs) -> BaseWorkflowResult:
        """
        Enables the current workflow instance to executed like a function. This method calls the `_run` private method
        under the hood to initiate the workflow.
