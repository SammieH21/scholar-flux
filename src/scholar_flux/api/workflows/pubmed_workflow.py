...
"""
The scholar_flux.api.workflows.pubmed_workflow module defines the steps necessary to interact with the PubMed API
and retrieve articles/abstracts. These two steps integrate into a single workflow to consolidate the two-step
retrieval process into a single step involving the automatic execution of a workflow.

Classes:
    PubmedSearchStep: The first of two steps in the article/metadata response retrieval process involving ID retrieval
    PubmedFetchStep: The second of two steps in the article/metadata response retrieval process that resolves IDs into
...
class PubMedSearchStep(WorkflowStep):
    """
    Initial step of the PubMed workflow that only defines the provider name as the current option, allowing
    overrides in place of other settings. The equivalent of this step is the retrieval of a single page from
    the PubMed API without the use of a workflow.
...
class PubMedFetchStep(WorkflowStep):
    ...
    These Ids are retrieved from the metadata of the previous step and are used as input to eFetch to retrieve
    their associated articles and/or abstracts.
    ...
    def pre_transform(
        ...
        """
        Overrides the `pre_transform of the SearchWorkflow step to use the IDs retrieved from the previous step
        as input parameters for the PubMed eFetch API request.
