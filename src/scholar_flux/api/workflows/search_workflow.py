...
class WorkflowStep(BaseWorkflowStep):
    ...
    config_parameters: Dict[str, Any] = Field(default_factory=dict, description="Optional config parameters for this step.")
    ...
    def pre_transform(self,
                      ...
                      config_parameters: Optional[dict] = None) -> Self:

        if ctx is not None:
            if not isinstance(ctx, StepContext):
                raise TypeError("Expected the ctx (context) to be a StepContext or None, "
        ...
        return self.model_copy(update = dict(provider_name=provider_name or self.provider_name,
...
class StepContext(BaseStepContext):
    ...
    step_number: int
    step: WorkflowStep
    result: Optional[ResponseResult] = Field(default=None, description="The response result received after the step's execution.")
...
class SearchWorkflow(BaseWorkflow):
    ...
    def _run(self,
             ...
             verbose: bool = True,
             **keyword_parameters) -> WorkflowResult:
        """
        ...
        try:
            ...
            for i, step in enumerate(self.steps):
                ...
                step = step.pre_transform(ctx, provider_name=step.provider_name,
                ...
                with search_coordinator.api.with_config_parameters(**step.config_parameters):
                    ...
                    ctx = step.post_transform(StepContext(step_number=i,
