...
class RateLimiter:
    ...
    def _validate(min_interval: Optional[float | int]) -> float:
        """
        Helper method that verifies that the minimum interval is a valid number
    ...
    def _wait(min_interval: float | int, last_call: float | int):
        """
        Helper Method that calls time.sleep() in the background to wait based on the current time,
        the previous function call, and the minimum interval between the two time periods.
    ...
    def __call__(self, fn):
        ...
        used to ensure that rate limits are not exceeded for APIs.
