# /api/rate_limiting/
"""
The scholar_flux.api.rate_limiting module defines the rate-limiting behavior for all providers.
The rate limiting module is designed to be relatively straightforward to apply to a variety of
context and extensible to account for several varying contexts where rate limiting is required.

Modules:
    - rate_limiter: Implements a basic rate limiter that applies rate limiting for a specified interval of time.
                    Rate limiting directly using `RateLimiter.wait`  or used with a context manager that records
                    the time of execution and the amount of time to wait directly.
    - threaded_rate_limiter: Inherits from the basic RateLimiter class to account for multithreading scenarios
                             that require the same resource. the usage is the same, but thread-safe
    - retry_handler: Basic implementation that defines a period of time to wait in between requests that are
                     unsuccessful. This class is used to automatically retry failed requests until successful or
                     the maximum retry limit has been exceeded. The end-user can decide whether to retry specific
                     status codes or whether to halt early.
...
Classes:
    ...
    - RetryHandler: Used to define the period of time to wait before sending a failed request with applications of max
                    backoff and backoff_factor to assist in dynamically timing requests on successive request failures.
