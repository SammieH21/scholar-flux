...
class SearchAPI(BaseAPI):
    ...
    def api_key(self) -> Optional[SecretStr]:
        """Retrieves the current value of the API key from the SearchAPIConfig. Note that the api key is stored as a
        secret key when available.
    ...
    def request_delay(self) -> float:
        """Indicates how long we should wait in-between requests. Useful due to comply with the rate-limiting
        requirements of various APIs.
    ...
    def api_specific_parameters(self) -> dict:
        """
        This property pulls additional parameters corresponding to the API from the configuration of the current
        API instance.
    ...
    def from_provider_config(
        ...
        **api_specific_parameters,
        ...
        settings associated with the provider config to temporarily make the configuration settings globally available
        when creating the SearchAPIConfig and APIParameterConfig instances from the provider registry.
    ...
    def from_defaults(
        ...
        **api_specific_parameters,
        ...
        """Factory method to create SearchAPI instances with sensible defaults for known providers. PLOS is used by
        default unless the environment variable, `SCHOLAR_FLUX_DEFAULT_PROVIDER` is set to another provider.
    ...
    def is_cached_session(session: Union[CachedSession, requests.Session]) -> bool:
        """Checks whether the current session is a cached session. To do so, this method first determines whether the
        current object has a 'cache' attribute and whether the cache element, if existing, is a BaseCache.
    ...
    def cache(self) -> Optional[BaseCache]:
        """Retrieves the requests-session cache object if the session object is a `CachedSession` object. If a session
        cache does not exist, this function will return None.
    ...
    def prepare_request(
        ...
        """Prepares a GET request for the specified endpoint with optional parameters. This method builds on the
