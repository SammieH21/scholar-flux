...
class ProviderRegistry(UserDict[str, ProviderConfig]):
    ...
    def __setitem__(self,
        ...
        if not isinstance(key, str):
            raise APIParameterException(f"The key provided to the ProviderRegistry is invalid. "
                                        f"Expected a string, received {type(key)}")

        if not isinstance(value, ProviderConfig):
            raise APIParameterException(f"The value provided to the ProviderRegistry is invalid. "
